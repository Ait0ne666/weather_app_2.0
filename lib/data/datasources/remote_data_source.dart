...
class RemoteDataSource {
  ...
  Future<Either<Failure, City>> getCityFromCurrentLocation(LocationPoint coordinates) async {

      try {

        final locationResponse = await http.get(options.locationNameByCoordinateRequest(coordinates.latitude, coordinates.longitude));
        
        if (locationResponse.statusCode == 200) {
          print(locationResponse.body);
          final currentCity = CityModel().cityNameFromJson(jsonDecode(locationResponse.body));

          print(currentCity);

          final weatherResponse = await http.get(options.weatherByLocationRequestUrl(coordinates.latitude, coordinates.longitude));      

          if (weatherResponse.statusCode == 200) {
            CityModel city = CityModel().fromJson(jsonDecode(weatherResponse.body));
            print(333);
            print(city);
